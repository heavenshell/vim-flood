...
function! s:run_callback(name)
  if flood#has_callback('check', a:name)
    call g:flood_callbacks['check'][a:name]()
  endif
endfunction
...
" Callback function for `flow check`.
...
function! flood#check#run(...) abort
  ...
  call s:run_callback('before_run')
